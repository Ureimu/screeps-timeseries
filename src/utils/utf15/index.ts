...
export interface CodecConfig<T extends boolean, U extends number[] | number, M extends boolean> {
    /**
...
export class UTF15<T extends boolean = true, const U extends number[] | number = number, M extends boolean = true> {
    meta: M;
    array: T;
    depth: U;

    /// Constructs codec by config or another serialized codec (this <=> cfg)
    constructor(cfg: CodecConfig<T, U, M>) {
        this.meta = cfg.meta;
    ...
    encode(arg: arrayType<T> | TypedArray): string {
        assert(
    ...
    decode(str: string, length_out?: LengthOut): arrayType<T> {
